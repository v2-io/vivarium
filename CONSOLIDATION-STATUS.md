...
2. **Component E's build** — a stage recording the $\varepsilon$ it reached. Design settled, retention bounded by a read primary; nothing in the tree records one yet.
...
4. **The live edge** — a poller plus surfacing epistemic state that is *already declared* (physics tier, provisional flag, statistic/exactness, maturity rung) and never reaches the viewer.
