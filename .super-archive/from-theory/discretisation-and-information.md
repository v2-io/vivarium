...
> **Graduated 2026-07-24** to `.super-archive/from-theory/`. **Do not re-mine as claim source.** Law + present-best teaching: `#disc-prime-question`, `#norm-bias-vs-noise`, `#obs-cube-locked-kernel-bias`, `#form-declared-structure-tradeoff`, `#detail-structure-scheme-map`, `#detail-info-theoretic-discretisation`, `#detail-fvm-control-volume`, `#worked-example-mfd-prime-question`, `#obs-routing-curl-spiral`, `#form-column-control-volume`, `#form-grid-equiangular-staggered`, `#sketch-detail-abstract-reversion`. §5–6 impact tables and §7 grid leads = history / open notes under grid segment; correction archaeology = history.
...
| **a potential** — ⚠ **CORRECTED 2026-07-13, AND THE CORRECTION IS THE FINDING.** *"Gravity-driven flow is curl-free"* is **literally false**: real discharge is $q=-K\nabla\phi$ with varying conveyance, so $\nabla\times q = -\nabla K \times \nabla\phi \neq 0$. **Real water on a real hillslope HAS curl. A probe built on "curl-free" would have convicted a perfect router.** The **exact, pointwise, mesh-independent** statement is instead: **the flow direction is orthogonal to the contours of $\phi$** ⇒ $$\kappa(C)=\oint \hat d\cdot d\ell \Big/ \oint d\ell = \langle \sin\Delta\rangle = 0$$ **on every closed CONTOUR.** $\kappa$ is *the fraction of transport running **along** the contour instead of **down** the slope*. $\kappa \neq 0$ ⟺ **the flow spirals** ⟺ the routed field is not the gradient flow of the potential it claims to be. **An Escher staircase** | **Claim home:** `#obs-routing-curl-spiral` (measurement + bound: $\kappa$ convicts fan-weighted direction, not $\hat Q_K$) · `#form-declared-structure-tradeoff`. **Present-best remedy stack (proposed, not adopted):** $p=1$, kill diagonals, Coatléven vector reconstruction on real face fluxes — **not** the mid-day first-moment tilt on outflow weights (superseded instrument that located the disease). Teaching: mimetic / structure-preserving discretisation (DEC, FEEC, compatible FD) as the structure class | **[M] ⛔ VIOLATED, AND MEASURED** — $\kappa \approx 2.0\times10^{-2}$ level-independent (see claim home for null-test and remedy stack) | ⛔ `erosion` routing — **CONVICTED** |
...
2. **The curl / contour-orthogonality probe.** Use the restated identity (table row "a potential") — **not** naive curl-free. Live home: `#obs-routing-curl-spiral`.
