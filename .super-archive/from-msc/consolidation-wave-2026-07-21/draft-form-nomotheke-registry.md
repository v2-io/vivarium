...
# PROMOTED — use `#form-nomotheke-registry` in core

**Promoted to** `core/src/form-nomotheke-registry.md` (design-wave integration).
This msc draft is no longer the claim home.
...
- Ice dual-homes once promoted: ARCHITECTURE §9 "adding a system" registry bullets → pointer; `nomotheke.rs` module header → pointer; leave per-nomos epistemic status strings as data, not re-prose.
- Sibling gaps: ASSUMPTIONS generated ledger; geometric contract as second declare/match surface; isostasy replacing uplift stub (proposed DECISIONS — not this claim).
