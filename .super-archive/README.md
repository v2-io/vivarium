# `.super-archive/` — fully integrated or superseded substrate

*Not claim canon. Not a tactical dependency. Not a second OUTLINE.*

## Program objective (read this)

**Main work of this phase:** peel claim gold out of residual substrate (especially **`.archive/` and live `doc/`**, and anywhere else prose still acts as a second mine) into **`core/` segments**, then **graduate** what is claim-empty or superseded here — so agents stop re-mining ice and stop treating old prose as law.

**Compounding:** every file that moves here **speeds the next adjudication** (smaller mine, fewer dual-homes, less re-litigation). Prefer **low-hanging fruit** when it is honestly claim-empty — do not save easy ice for later while re-reading it every pass.

**Integration is replacement** — the claim’s home becomes **core segment body**, **Working Notes** on a segment, or **this directory** (super-archive). It does **not** mean “leave a live pointer table.” A live file that is only “claim home: #slug” links is dual-home cosplay / a second OUTLINE. Unpealed teaching residual may stay in live `doc/` until pealed.

There is **no path ban.** Graduation rules apply to **any** tree path: `.archive/`, `doc/` (design, theory, plan, ARCHITECTURE, …), even **`ref/`** if a reference note is fully superseded and claim-empty. “Still live under X” only means **not pealed clean yet** (or still the useful place to *consult* primary literature), not “excluded by design.”

Practical bias (not a proscription): **`ref/`** is usually *true reference* (PDFs, NOTES, surveys) that stays consulted rather than onboarded — it often has no claim residue to graduate. That is a *default lifestyle*, not a rule forbidding `ref/` from this directory.

Earlier wording that banned `doc/` (or any whole tree) from super-archive was a **mistake**. Corrected 2026-07-23 (Joseph).

## What this is

Material that has **already been promoted into claim homes** (`core/src/` segments, LEXICON, DECISIONS, ETHICS, ordinum, ASSUMPTIONS, PROCESS) and/or is **superseded by present segment truth**. After graduation: provenance / rewind only; **do not re-mine as claim source**.

| Layer | Role |
|-------|------|
| **`core/`** | Sole claim truth |
| **Residual mine (any path)** | **Mine field** — extract → segment (or ice) → graduate when claim-empty |
| **Live instruments / primary literature** | Spikes/probes that still *convict*; `ref/` PDFs and NOTES while still the right place to consult — stay live until that job ends |
| **`.super-archive/`** | **Done mining for claims** — integrated or superseded |

Suggested layout as batches grow (optional subdirs, not required):

- `from-archive/` — graduated from `.archive/`
- `from-design/`, `from-theory/`, `from-plan/`, `from-architecture/` — graduated from `doc/…`
- `from-ref/` — only if a `ref/` prose note is fully superseded (rare; not proscribed)

## Graduation rules (all must hold)

1. **Claim homes exist** for every load-bearing assertion the file was carrying — or the file is pure process/history with no remaining claim residue (honest ice).
2. **No tactical dependency** from main tree code or segments on that path.
3. **MANIFEST entry** names path, date, superseding slug(s) or “pure ice / re-founding specimen.”
4. Prefer **`git mv`**; update live pointers that still cited the old path (one-line claim-home or remove).

**Not ready *yet*** (same rule for every path): still has unextracted gold; or is the **primary instrument** for a live measurement (e.g. spike `RUN.txt` until numbers live only in segments / DECISIONS citations). That is a *readiness* check, not a forever ban.

## Novel material that is not ready for primacy

Not everything pealed from the mine is primary-outline law. For **novel** ideas that **do not supersede** existing segments but are **not ready for primacy** yet, prefer:

- **Working Notes** on the related primary segment (forward pointers, open tension, do-not-reclaim), or  
- a **`type: sketch` / exploratory segment** (honest stage; may sit lightly on OUTLINE or as a named gap), or  
- **`msc/`** notes/spikes when it is instrument or session trail rather than claim shape.

Do **not** leave such material forever in dual-home design prose *as if* it were residual mine gold for primacy, and do **not** over-promote it to exact Critical just to clear a file. Super-archive still takes the **source file** once its *load-bearing* claims are owned or iced — sketch homes can hold the novel remainder.

## Relation to audits

`audits/.integrated/` is for **audit reports** after verified/adjudicated/integrated (`#ops-audit-integration`). This directory is for **prose substrate** (archive + design + theory + plans + …).

## Index

See [`MANIFEST.md`](MANIFEST.md). Newest batches first.
