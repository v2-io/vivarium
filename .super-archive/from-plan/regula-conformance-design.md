...
| **nomos declaration** (nomotheke, built) | one algorithm's epistemics: tags, deps, promises-with-conservation, assumption anchors | algorithm | data + minted keys + tests |
| **slot** (new, small) | a named system-role in the world-assembly, identified by its **flux interface** (ARCHITECTURE §9): what any occupant must produce/consume. Nomos are candidate occupants at different rungs | system-role | today: the nomotheke's `system` field, promoted to first-class |
...
Joseph's standing assumption, adopted: the phase charges are a **floor until proven otherwise**. Codified: the **Ordinum** (full formal name *Ordinatio Ordinum* — "the appointed arrangement of the orders [of kingdoms]"; cited short as Ordinum, keyed `ordinum@version`) is the per-phase Charge/Promise floor **as data**, derived from `.super-archive/from-archive/PHASES.md` — which is thereby the Ordinum's **reportatio**: the prose working text the authorized version is compiled from (the medieval ordinatio-vs-reportatio carve, Scotus precedent). Once codification lands, `.super-archive/from-archive/PHASES.md` is the honest first-guess prose (archived 2026-07-11).
...
**Built 2026-07-11: `tabularium/terrestris.ordinum.udon` (Terrestris v0.1.0)** — the first instituted artifact of the **tabularium** (the library of structured-but-not-Rust artifacts Rust consumes via libudon). *Terrestris* names the earthlike world-kind — the **track**; its floor (this ordinum) and its profile (Regula Terrestris) share the track name, told apart by artifact type (`.ordinum.udon` vs `.regula.udon`) — one track, two faces (`:manifold cube-sphere-3d-voxel`). It is one ordinum among a planned many (a cellular-automata / 2-D testbed is the anticipated second — the *schema* is world-kind-agnostic, only the content Earth-lineage). One ordinum, many regulae: **Regula Terrestris pins it**, and other profiles over the same ladder are possible. In the codified form a phase declares **charges** (gating), **promises** (the forward superset — typed `.class` ∈ state/regime/capability/limit, each optionally `:kept-by` a nomos and `|predicate`-checked; no link = honest gloss), and explicit **defeasances** (retirements of prior promises; inherited promises continue implicitly). Version lives in `:version`, not the filename (a fork is what mints a versioned filename). `.super-archive/from-archive/PHASES.md` is the reportatio (archived 2026-07-11).
...
.super-archive/from-archive/PHASES.md design notes carry the principle in prose: a canceling pair (magnetic field × solar wind; Jupiter-shepherding × impactor flux; the deferred moon) "stays jointly unbuilt at zero fidelity cost **until a consumer queries the difference or either member alone**." That condition is mechanically checkable against the nomotheke:
...
**Structural prerequisite found in code (2026-07-11):** `NomosDecl` declares deps (nomos→nomos) and promises (quantities out) but has **no `consumes` field** (quantities in) — and permit-voiding is defined over consumed quantities. v0 adds `consumes: &[&'static str]`, with quantity names as shared constants referenced by both promises and consumes, so a typo fails the build (same pattern as the ASSUMPTIONS anchors).

**The requisite closure is a planning query, not a discovered wall.** The audit computes the transitive closure of a *proposed* nomos before it lands: every consumed quantity must be either promiseed by an occupied slot or covered by a live permit, and the report prints the full requisite tree (aurora → magnetic field + solar-activity cycle; UV-at-surface → ozone → atmosphere composition — Joseph's named cascade). Tool form, later: `vivarium audit --add <nomos>` — "if I add this, here is what the regula then demands." This converts "there will be points where we cannot make the law self-consistent without prior world-level requisite nomos" from an incoherence someone trips over into a build planner.

**The motivating specimen is already in the store**: `water-tile` consumes precipitation with **no atmosphere slot filled** — rain without a sky. Under any regula over the Terrestris ordinum this is a voided-permit condition *today*. The pre-regula world runs, in effect, permits nobody wrote down; the regula's first job is to write them down (grandfathered, dated, with their retirement conditions — the reservoir layer).

## 4. In-vivia experimental rigor — results are memos with cones

The payoff Joseph specified: find a latent flaw in a nomos → see which experimental results it touches, at better-than-academic latency (his bar: patch, rerun, analyze next day). The mechanism is already built, just not yet applied to results: **an experimental result is itself a memo** — a query over the world plus an analysis nomos, with a complete key naming its whole dependency cone (vivium manifest → regula@version → nomos@versions → seed → the keys it read). Two latencies fall out:
...
## 4c. Promise conformance — the fulfillment audit (Joseph's acceptance-test discipline)

The ordinum's typed promises turn conformance into a computation the audit emits, promise by promise:

- **Promises in force at phase N** = `⋃(promises 0..N) − ⋃(defeasances 0..N)`. A phase declares only its *novel* promises; inherited ones continue implicitly; a defeasance explicitly retires one. So the standing forward-interface at any phase is derived, not re-listed.
- **The maturity ladder, computed per promise**: named (prose only) → specified (has a `|predicate`) → claimed (has `:kept-by`) → kept (the predicate passes against the kept-by nomos at a declared tier, via declared-vs-derived). The audit prints each promise's rung beside the fidelity pyramid.
- **The acceptance-test rule** (Joseph, 2026-07-11): *a promise with no predicate cannot be marked fulfilled at any epistemic level* — nothing has been defined that could convict it. The audit surfaces these as **`promise X: un-checkable — no predicate`** findings. This is what lets big-picture promises land first and their predicates be written when we actually need to see them delivered — which then *guides the coding* (the ordinum is the spec).
- **The incoherence rule** (same shape as a voided permit): a **defeased promise whose `:kept-by` nomos is still consumed by a live nomos** is a conformance error — a later phase retired a promise that downstream code still leans on.

All four ride on machinery that exists (the store's cone, the nomotheke's declarations) plus the ordinum's typed promises; no new bookkeeping, only the fold.
...
- The Ordinum derived from .super-archive/from-archive/PHASES.md **for the target phase only** — the aspiration-dump guard: a regula reaching for Phase 8 makes every report a wall of red, and an ignored dashboard is worse than none. Later phases render as *horizon*, never as failures. The lineage's `target_phase` advances as the work does.
...
**Supremacy clause (load-bearing, stated once):** the **moratorium is not within the regula's gift** — it is program law (`ETHICS.md`, the Standing Moratorium Imperative) sitting *above* the regula layer, a constraint on the space of valid regulae. No regula can permit endo instantiation of emergence-capable substrates; the conformance audit checks it mechanically. The regula is the moratorium's *enforcement locus*, never its negotiation surface. Terrestris carries it explicit-and-compliant; ASF §0's "for now" is load-bearing and names its own revisit conditions — lifting is future-us's to *earn* (vouchable Lawfulness, reversibility, consent-by-proxy stewardship), and no work here forecloses or presumes it.
...
- **Register convention** (observed, kept deliberately): **Greek** names the law-article machinery (*nomos, nomos, nomothete, nomothetic, nomotheke*); **Latin** the world-governance layer (*regula, regnum, ordinum*); **plain English** the lived concepts (Kingdom, Charge, Promise, Record, slot, permit). Register signals layer; a proposal that crosses registers should notice it is doing so.

## 7. Critiques (honest, held against the design)

1. **Prose-wish risk**: a regula that isn't computed against the census is .super-archive/from-archive/PHASES.md with extra steps. Mitigation is constitutive: it ships only as data + conformance function + tests, like the nomotheke. No prose-only regulae.
2. **Prematurity risk**: three nomos, one representation kind — is a conformance layer YAGNI? Countered by the live specimen (§3): the rain-without-atmosphere incoherence exists *now*, and the reservoir/thermal work needs a specification anyway — the regula is that spec. Thin v0, no machinery beyond the first real check.
