...
- **Incidental evidence that exchange influence crosses a tile boundary — not the owed cone probe (2026-07-30).** Repairing the chart-edge halo ( #obs-chart-edge-halo-clamps-to-the-face ) changed every off-chart value in the halo bands of roughly 168 perimeter tiles, and the rebuild moved **249 `erosion-tile` roots: 224 on perimeter tiles and 25 on INTERIOR tiles whose own windows do not overhang at all**. An interior tile can only have moved by receiving a perimeter neighbour's corrected band through the Jacobi exchange, so influence demonstrably spreads at least one tile. Read at that strength and no further: this is an uncontrolled by-product of a repair, with the perturbation neither localized nor sized, so it **does not** measure the cone radius, does not price $\rho$, and does not replace the probe FE(5) owes.

- **And the halo band matters to the published interior less often than "the window overhangs" suggests (same rebuild).** Of the ~168 tiles whose halo bands changed, only **35 distinct tiles** published different heights — about **21%**. A pre-registration predicting 30–45% of payloads missed at 7.0% for exactly this reason: overhang is *necessary, not sufficient*, because the band reaches the published cells only where the difference propagates through routing far enough to arrive, and at $d = 16$ under cone truncation it usually does not. Same caveats as above — one seed, one grain, a perturbation confined to the outermost band — so this is a first datum on how much of a window's halo the interior actually feels, not a law about $d$.
