...
- **External re-read of FE(7) (dossier 2026-07-29, suggestive-not-law):** mean-elevation error across all six exchange arms is strictly monotone in the single group $d/(v\sigma)$ (with $v \approx 1$ cell/epoch inferred from FE(2)'s front), collapsing at the causal-cone threshold 1 — the overlapped-tiling exactness condition $d \ge \sigma s$ wearing our dials. It does **not** organize mean $\lvert\Delta h\rvert$, exactly as the chaos floor predicts. Arithmetic verified against FE(7)'s table; falsifier = matched-ratio arms on `halo_exchange_probe` (same run measures $v$ and sizes the FE(9) floor). Full reading: `ref/research/seam-exchange-precedents-dossier.md` §2.2–2.3, §4.1; forward item (b) above is subsumed by this probe design.
