...
6. **A neighbour-dependent contract is keyable, and the third value has a thin production path.** The two named single-tile contracts are both functions of the tile alone, which is why neither is honest at a coastless window (FE(1)); the third — a halo exchanged with the neighbours the tile actually has — is what supplies a base level that is not a fiction ( #form-same-level-halo-exchange ). Its declaration is a **descriptor** (`HaloSchedule`: depth, cadence, cone truncation), each field identity the moment a caller can vary it. The descriptor keys at constant size ( #form-complete-content-addressed-key FE(6)). Schema shape: `Undeclared | BaseLevelSink | NoFluxWall | Halo{d, σ, ρ}` — Halo is selectable through `erosion_region_exchanged`; the first two remain probe/kernel-selectable; NomosDecl box schema is still unbuilt.

7. **Out of bounds for this segment.** Full NomosDecl boundary-contract column; choosing operating $(d,\sigma,\rho)$ as law (measured at one grain only — #obs-exchange-repairs-the-seam-and-overlap-does-not ); claiming the default planet build uses exchange — it does not yet.
...
- **Keyed selection — Halo half landed 2026-07-29.** `World::erosion_region_exchanged` writes store citizens under `HaloSchedule` keys; single-tile `BaseLevelSink` / `NoFluxWall` are still not in the default key (deliberate — no silent rekey). Remaining instrument gap: default builder and view cohort still use edge-sink keys, so the planet you open is not yet the exchanged one. Era chain-carried still wants this dependency story first.
