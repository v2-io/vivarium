...
5. **Cube-locked structure control (mandatory).** The grid *manufactures* cube-aligned bias when kernels assume a flat uniform Cartesian tile: measured specimens include (a) MFD fan attractors along face $u/v$ axes, (b) uniform cell-area drainage overstating $A$ cube-lockedly (~+18% mean, up to ~+41% at edge midpoints), (c) organised spurious vorticity with a null island at each face centre. **If emergent plate boundaries or drainage structure correlate with cube-face edges/corners, the result is void** until the bias is ruled out — a seductive false triumph is the failure mode.
...
- **Price unpaid:** face-keyed store + leaf-only evolution vs double-evolve / mean-pin — see #form-rl-closure-algebra compliance debt and #form-seam-flux-exchange .
