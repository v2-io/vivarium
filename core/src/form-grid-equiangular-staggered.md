...
- **Sweep breadcrumb (self-destructs).** `msc/spike-principled-router/ and msc/spike-router-pricing/` — the router investigation this segment's open-router clause summarizes. Likely primary absorbing segment: this one; the mapping is asserted from session memory, not verified — at the msc graduation sweep, *verify* the residue is fully absorbed here (or land what is missing, or re-point this note if the mapping is wrong), then graduate the residue to `.super-archive/` and **delete this note** (integration-is-replacement: the note's disappearance is the done-signal).
