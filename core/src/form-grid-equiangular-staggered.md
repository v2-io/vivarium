...
5. **Cube-locked structure control (mandatory).** The grid *manufactures* cube-aligned bias when kernels assume a flat uniform Cartesian tile. Measurement home: #obs-cube-locked-kernel-bias (MFD fan; uniform cell-area $A$; control law). Related routing curl/potential-identity still a residual gap. **If emergent plate boundaries or drainage correlate with cube-face edges/corners, the result is void** until those biases are ruled out.
