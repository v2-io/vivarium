...
- **Live modules:** `mantle_thermal.rs` (cooling trajectory $T_p(t)$, the chain head); `lithosphere.rs` (column + `freeboard_m` + `freeboard_m_at_tp`; `reference_m_at_tp` memoized per `(seed,T_p)` — the O(N²) trap the cooling pour would otherwise fall into); `sea_level.rs` pour (seed-only + `derived_sea_level_at`/`_at_tp` epoch chain); `uplift.rs` **rate only**; nomotheke `MANTLE_THERMAL` → `LITHOSPHERE` (consumes `mantle potential temperature`) → `ISOSTASY`; erosion consumes `ROCK_UPLIFT_RATE` and emerged-land geometry from isostasy.
