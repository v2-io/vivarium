...
5. **Not fully built.** Live `spec` / roots may still carry a thin subset (name, seed, format). Full field set is **designed; partial implementation** — this segment owns the *architecture*, not a claim that every field is executable today.
...
Stage `draft`. Sources: DECISIONS regula-collapse; `.archive/VIVARIA-DEFINITIONS.md` Layer 3 (residual mine until demoted); regula-conformance-design as reasoning trail only.
