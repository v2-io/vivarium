...
3. **No regula artifact (for now).** A separate Regula / Slot object is **not** required for conformance audit. Promise fluxed-quantity + `:kept-by` carries what “slot” reached for; permits live on the manifest. Reach for an in-between noun only if the two-artifact scheme actually strains (`DECISIONS[regula-collapses-to-order-and-manifest]`, `:by us`, decided). **The line to hold under the collapse:** rigor-floors live on the manifest/template, never baked into the ordinum — the ordinum states what the *kind* requires; how rigorously a given world must meet it is per-vivium prescription (the collapse's design carried this clause; it travels here so the collapse does not silently flatten it).
...
- **Sweep breadcrumb (self-destructs).** `msc/build-parameterization-findings-2026-07-24.md` — the ripest graduation: all four items demonstrably landed (frames, demand bucket, the relocation no-go, water's 200), each with a DECISIONS row citing it. Likely primary absorbing segment: this one; the mapping is asserted from session memory, not verified — at the msc graduation sweep, *verify* the residue is fully absorbed here (or land what is missing, or re-point this note if the mapping is wrong), then graduate the residue to `.super-archive/` and **delete this note** (integration-is-replacement: the note's disappearance is the done-signal).
