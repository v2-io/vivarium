...
- **Feature lift for tree ensembles (Joseph, 2026-07-29, from prior practice):** trees split axis-aligned, so pre-compute logarithmic / power / convolution transforms of the raw signals as extra features — a kernel-style lift that stops the forest straining to approximate multiplicative structure with staircases. Directly apt here: the physics is largely products of powers (stream power, Manning), so log-features turn the true laws into *single splits*, while the ensemble's native robustness to noise, confounders, and un-normalized inputs (the reason to prefer it over NNs at this stage) is untouched. The lift also feeds FE(6): engineered features participate in leaf-coverage rarity like any other.
