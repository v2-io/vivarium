...
- **The remaining class needs a balance criterion**, not a stationarity one — something of the form *incision rate matches uplift rate to within a declared fraction, tile-wide*. Still open as a *criterion* — but the literature sidebar (2026-07-28, `msc/research-lem-sota/`) reframes the count question it guards: the field's own conclusion is that numerically-detected steady state is unreliable and the honest quantity is the **a-priori analytical response time**, which at $n=1$ is computable from static inputs with no calibration ($T_A$ independent of uplift — Whipple & Tucker 1999). `Fluvial::response_census` (`examples/response_time_probe`) is the first instrument for it: on the live world's landiest L9 tiles the wave crosses the channelized network in ~20–70 epochs (so the authored 40 is one channel-crossing, and the still-falling 400-epoch residual of FE(5) is the *hillslope* clock, a different and slower one); candidate L13 patches measure ~300 epochs max with per-epoch Courant ≈ 1 at trunks. The χ-shape test (Perron & Royden 2013) is the candidate *criterion* form — a shape statement a driven steady state satisfies, immune to the driver-bound residual — proposed in the sidebar, unbuilt.
