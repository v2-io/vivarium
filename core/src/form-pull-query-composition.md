...
6. **Out of bounds for this segment.** (a) Three-scoped runtime (coarse spine + local cones + edit-propagation) — claim home #form-three-scoped-runtime ; (b) predictive prefetch (same home, FE(6)); (c) “time in every key” as fully mechanized DAG (same home, FE(3) — doctrine, unenforced); (d) edit up-invalidation / irreducible-edit closure ( #sketch-detail-abstract-reversion ); (e) independent coarse-tier evolution vs leaf-only flux identity (open tension with #form-face-flux-register ); (f) restating complete-key or save≡store as if new here.
