...
1. **Canonical key.** `CellId(u64)` — S2-style packing of (face, level, Hilbert distance); 6 faces, quadtree per face, levels to ~25 (≈ 0.5 m footprint). Exact, drift-free (no far-lands float origin), hashable, and Hilbert-ordered so a region is a contiguous id range (storage/streaming locality). Parent = bit-shift, so the LOD ladder is structural. It is the spatial component of every complete content-addressed key ( #form-complete-content-addressed-key ). Live: `sphere.rs`.
2. **The curve orders chunks, not cells.** The Hilbert curve's job is container ordering. A **patch** — the automata/storage working unit — has a plain Cartesian row-major interior where neighbours are `idx±1`. Measured basis (spatial-key bench): Cartesian neighbours ~80× faster than Hilbert-id neighbour arithmetic; the stencil runs at ~6 Gcells/s while a patch fits cache. Curve encode/decode is designed out of every hot loop.
3. **Two size tiers.** A coarse macro/streaming **chunk** (tens of km — an LOD container, undense-able at fine levels) and a fine automata **patch** at the cache sweet spot (~0.25–0.5 km, 256²–1024²). Live: `chunk.rs` (patch + halo).
4. **Halos carry the seams.** Cross-patch and cross-cube-face neighbours (with the face axis transform) are materialized into a ghost border at patch load, so the inner loop never branches (domain decomposition + halo exchange). A halo is a *read* surface; it does not make neighbour state a mutable coupling channel ( #form-column-control-volume seam duty still binds).
...
**Known incomplete:** the two-tier chunk (macro streaming container) is thinner in code than the patch tier; face-keyed flux objects ( #form-grid-equiangular-staggered FE(2)) are not yet store citizens; the ~20 km livable-shell vertical carve is design prose, not a built boundary.
