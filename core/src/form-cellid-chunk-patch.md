...
**Known incomplete:** cross-cube-face halo fill (FE(4)) is spiked-and-measured, not built — same-face fill only, and the depth-$\ge$2 resampling choice FE(4) names is undeclared; the two-tier chunk (macro streaming container) is thinner in code than the patch tier; face-keyed flux objects ( #form-grid-equiangular-staggered FE(2)) are not yet store citizens; the ~20 km livable-shell vertical carve is design prose, not a built boundary.
