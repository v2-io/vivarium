...
8. **Instrument / ops notes (revised 2026-07-31).** Not leverage rank:
   - **Explorer LoD + performance (Joseph requisition 2026-07-29) — first solid pass 2026-07-31.** Pathologies fixed: (1) zoom while a pull was in flight dropped all newer requests → instrument built the *wrong* view slowly; now latest-wins drain on both sides. (2) Camera always looked at planet centre → horizon mush at low altitude; now blends to nadir. (3) Whole-globe ceiling was L9 (~1.6 M cells); now L7, with region windows earlier and sticky. (4) Auto-level jumped many steps; now steps at most one level. Residual: no multi-resolution shell under a fine window (planet still disappears outside the patch); true quadtree still open.
