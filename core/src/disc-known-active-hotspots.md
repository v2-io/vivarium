...
5. **Highest-leverage next — revised 2026-07-29 (late evening), after the ocean-connectivity land.** Ideation rank only.
   ...
   **What moved.** Rank 1 was *water settle path → lakes on screen*. It is **retired as the lake blocker**: lakes did not need the settle. The sea was classified by height, so every landlocked below-datum basin was a drain ( #form-ocean-is-connectivity-not-elevation ), and repairing that put **1165 standing bodies / 976 865 km³** on the planet with no kernel step at all ( #obs-connectivity-fills-the-basins-the-threshold-drained ). The settle question survives for transients, currents and flood propagation — not for standing water, and #obs-water-fill-never-settles now says so. Three consequences reorder the table below: the **mixed-tier reader** is what stands between a viewer and an honest lake; the **ocean mask** wants to be a shared object before four consumers drift further; and the **spill/sill object** of 1b gained a second, independent motivation.
   ...
   | **1** | **Make one phase-3 promise convictable.** `vivarium status` already prints the whole ordinum ladder with each promise's rung and a verbatim `(un-checkable — no predicate)` where there is none; `emerged-land` is the one promise with a Record predicate that convicts (`sea_level::emerged_land_verdict`). So the machinery and the precedent both exist. What is missing is that **nothing gates on the ladder** and **no promise can reach KEPT while its predicate is prose** | The honest ordering question — phase 4 promises are `claimed` while phase 3's `closed-water-cycle` sits at `specified` and `weather-channels` at `NOT-STARTED` — is already *visible* on every `status` run. Turning one of those into a probe that can fail is what would make the floor load-bearing rather than reported | Not writing a regula artifact, and not declaring a target phase: the manifest declares `order = "terrestris"` and `target_phase = 4`, and `status` computes the rungs. Read `vivarium status` before proposing anything here |

   **On the phase floor, and read `vivarium status` first — it answers most of what one would otherwise guess.** It prints every promise's rung on the ordinum ladder (NOT-STARTED → specified → claimed → KEPT), its keeper, and `(un-checkable — no predicate)` where the ordinum's own maturity rule applies: *"a promise with NO predicate CANNOT be marked fulfilled at any epistemic level."* So `weather-channels` at NOT-STARTED is the ladder working, not a hole in it, and there is **no false green** anywhere on the floor. What the output does show is the real ordering tension: phase 4's `emerged-land` and `erosion-substrate` are `claimed` while phase 3's `closed-water-cycle` is only `specified` and `weather-channels` has not started — with `abiogenesis` carrying *"Joseph: Abyssal opens after this exists"* and this world holding no life. That is reported on every run and gated by nothing, which is the gap rank 1 names.
