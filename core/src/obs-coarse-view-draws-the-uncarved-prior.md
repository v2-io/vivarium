...
6. **The opposite direction is not symmetric, and it is the more dangerous of the two.** Asking a region *finer* than its level does not fall through — `grid_pos` accepts any `level >= self.level`, and `surface_m` answers with a bilinear read of the carve **plus the fine prior's detail re-added** (`gen::initial_topography_m(cell, level) − gen::initial_topography_m(cell, self.level)`), which is the fidelity ladder working as designed ( #form-fidelity-ladder ). So a view at L13 over an L9 carve is not blocky and is not obviously coarse: it is full of correct, lawful, kilometre-scale relief.

   **None of that relief was produced by a fluvial run at that scale.** The valleys are prior detail draped over an interpolated carve; there is no drainage network behind them, no discharge that cut them, no knickpoint that retreated up them. An eye trained on real landscapes reads them as fluvial, because at that scale that is what fluvial form looks like — which makes this the *coarse* case's failure with the safety removed. The coarse case at least yields a picture whose disagreement with the store a provenance glance exposes; here provenance says "eroded" and it is telling the truth. Only the **carve tier** — the level of the region that actually answered — distinguishes them, and until 2026-07-28 nothing reported it. `vivarium explore` now counts drawn cells by answering tier and names the coarser fraction; the query path still returns no tier with its answer (Working Notes).
...
- **The honest minimum is a report, not a fix:** any read that falls through to the prior while a covering tile exists at a finer level should be able to say so. Today the explorer names it on the HUD; nothing in the query path returns it, so every other consumer is on its own. **The same gap, in the FE(6) direction, is now the sharper one**: `surface_at` returns an `f64` and nothing else, so a caller cannot tell a carve at its own level from a carve two levels coarser with prior detail re-added. The explorer reconstructs it by calling `tier_at` per cell alongside `surface_at` — two traversals of the region list where one would do, and a discipline every other consumer would have to re-invent. A `surface_with_tier` returning `(f64, Option<u8>)` would close both directions at once.
