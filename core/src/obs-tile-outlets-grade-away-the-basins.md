...
The partial-tile outlet policy makes every edge cell a sink, so each tile grades itself to its own perimeter. **Measured: no tile-local drainage basin can exceed its tile — assembled routing of the L13 beacon patch finds a 3.85× larger trunk — and assembling tiles that were graded to different perimeter heights puts 13% of the patch in closed depressions that are seam artefacts.**
...
**The whole-face control has been run** ( #obs-fill-writes-itself-into-the-bed ), and it answered FE(2) and FE(5) in the direction opposite to the expectation: a whole-face coast-only carve holds *fewer* closed depressions than any tiled carve, so the 12 cells of FE(2) are neither geography nor evidence for this policy — they are deposition pits in a small population that the next epoch's fill erases. That is why FE(3) now attributes the absence to the fill. **What that control also could not test, and what remains the honest gap:** no *sub-face* window can be carved coast-only, because the policy is inferred from `origin == (0,0) && nx == 2^{level}` rather than declared ( #form-declared-boundary-contract ), so FE(4) and FE(6) have never been run against their own alternative at their own level.
