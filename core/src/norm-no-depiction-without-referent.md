...
5. **The failure mode this exists to forbid, stated concretely.** A system whose store holds only some of its states must not be animated by interpolating between them. That would synthesize exactly the smooth evolution a viewer is watching *for*, and it would be a renderer's guess wearing the appearance of a result. Where a system has no interior, the honest depiction is a discontinuity plus a statement that there is nothing between — and the correct repair is in the **builder** (materialize the intermediate stages), never in the view.

   **The repair has now been made on the case this clause was written against, and the clause survived it intact.** `erosion-tile` carried one distinct time-index when this was written; it now carries a materialized settle history, and the explorer scrubs it ( #form-time-indexed-stage-chains FE(11)). Eight stages over forty epochs is a slideshow, and the pull toward tweening is therefore *stronger* after the repair than before it — the builder has made the sequence just continuous enough to look like it wants smoothing. It is still forbidden, for the reason FE(4) gives: a tween is designed to look like continuous physics, so no label can stop the eye from reading it as motion. What the view does instead is state, on the stage it is showing, which epochs it sits between and that nothing exists there.

   The same repair sharpened the clause's *converse*. Erosion's chain is **materialized-only** ( #form-time-indexed-stage-chains FE(8)), so a request for more stages is a build request; the mantle-thermal chain is **law-evaluable**, so a denser sampling is more evaluations of a closed-form law and manufactures nothing. Both are legitimate and they are not the same permission. A view that carried one density dial across both would be lawful on one chain and inventing on the other, with no visible difference between the two cases.
