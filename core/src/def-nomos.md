...
**Version honesty.** Versions are part of the complete content-addressed key. Hand-stamped version strings remain the human-legible algorithm-era label, and they are now backstopped by a **build-time whole-crate source digest** (`build.rs` → `VIVARIUM_SRC_HASH`, folded by `NomosDecl::key`) that changes when any kernel source changes — so the convict-when-the-algorithm-changes-without-a-new-key property of #norm-declaration-must-convict is met at whole-crate granularity, and a forgotten hand-bump can no longer serve a stale memo ( #form-complete-content-addressed-key FE(4)). Bit-perfect *per-nomos* source attribution (and IR-normalized hashing, #lexicon/term/generator-pinning) remain open design, not claimed as built.
