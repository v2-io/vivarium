...
**Known compliance debt:** **plain single-tile** keys still seed drainage from own area and use edge-outlets (`EdgeContract::BaseLevelSink`); that is the probe/legacy article, not the builder default. **Production face/beacon sweeps** use Jacobi halo for the local boundary datum ( #form-same-level-halo-exchange ); sill-graph + discharge flux **first wires landed** on that path (key `sill1,flux1`); sediment flux / C+1 exactness / store memo of flux vector still open. `seam_ridge` is red by design (measures the honesty gap); production face-keyed flux register and leaf-only evolution unbuilt (measured in spike — #form-face-flux-register ). Debt does not soften the law ( #norm-declaration-must-convict ).
