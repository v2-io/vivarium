...
**Scope, and therefore not claimed.** Two levels (L9 on three faces, L11 on one), two epoch counts (40 everywhere, 300 on f0 at L9), one seed. The whole-face arm is not an unbounded domain: it still has four cube-face edges, and treats them as a **no-flux wall** (non-outlet, no receiver outside the field) — a different undeclared boundary contract, not an absent one, so FE(5) says the basin cap moves from the tile to the face, never that it is gone. The beacon regime (L13, 78 km tiles, 300 epochs) is measured *separately* and is not this segment's claim — it is homed at #obs-tile-outlets-grade-away-the-basins FE(5), and it goes the other way: there the two contracts end 274–297 m apart and the tiled carve puts 60.6 % of the window in closed depressions. So FE(4)–(8) here are statements about the **planet build**, and the honest summary across both is that the contract's consequence is a strong function of grain-against-geography rather than a constant. Whether the fill's raise should be restored before incision — the repair FE(1) implies — is neither built nor costed here.
...
- **The scale argument is now measured on both sides, and the parameter it needed exists.** `EdgeContract` ( #form-declared-boundary-contract ) makes the contract selectable on any window, which is what let `examples/beacon_contract_probe` carve the beacon footprint both ways. It is a *kernel* parameter and deliberately not yet a *key* field: nothing reached through `query.rs` can choose one, so the builder's complete key stays complete. The day a keyed caller can choose, the key gains the column — and a tile whose contract makes it depend on its neighbours has to fold that dependency in as well ( #form-complete-content-addressed-key ).
