...
- **A third reason, and it needs no refinement mismatch at all** (measured 2026-07-29, reproduced before landing): on a *conforming, same-level* cube-panel seam the two-point flux Laplacian has O(1/h) local relative error (doubling per rung), carrying ~99.5% of the whole-sphere L2 defect while the face interior converges — the measured order −0.5 is this seam ring, arithmetically ($O(1/h)$ on a codimension-1 set → $h^{-1/2}$ in a 2-D norm). Lipnikov's coarse–fine non-convergence and the bulk non-orthogonality inconsistency are the other two; this one is a property of the panel seam alone (`DECISIONS[the-two-point-flux-defect-is-a-panel-seam-defect]`; obs segment owed).
