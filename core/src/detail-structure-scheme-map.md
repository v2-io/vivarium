...
- **The discrete GCL is unchecked, and it is a fork, not a tax** (survey §4.9, read primary): on a curved mapped mesh, well-balancedness and entropy conservation are *lost as theorems* unless the discretely-computed metric terms satisfy the geometric conservation law (Wintermeyer et al.) — "metrics true" is not "metrics discretely consistent," and **we have never checked a discrete GCL**; the probe is cheap and named ( #disc-open-problem-census ). The 2026 escape: a *covariant* formulation (Montoya et al., `montoya-2026-covariant-sphere`, cubed-sphere experiments) makes the GCL requirement never arise — and our closed-form Jacobian is exactly what makes that route cheap. Conditions this segment's WB and entropy rows.
- **The ABF RT0 trap, and the highest-value paper check the survey names** (§4.8, read primary): RT0 — "one flux per face" — has *no convergence of the divergence* on non-affine quadrilaterals (Arnold–Boffi–Falk 2005), and cube-sphere cells are non-affine quads; this may be the theorem behind the grid report's *measured* "two-point flux inconsistent, order −0.5" (correspondence **not proven** — FE-basis statement vs FV flux). Bochev–Ridzal's rehabilitation is the natural mimetic divergence = the FV divergence, so a face-flux FV scheme may *already be* the rehabilitated form — an inference, checkable on paper, and good news if it holds. Survey home: `msc/research-structure-preserving/README.md` (keep-live: the designated survey-detail layer of #disc-prime-question ).
- **Residue map for the survey pair** (verified 2026-07-29, replacing the from-memory breadcrumb): `msc/spike-corrected-scheme-cost/` is fully absorbed in `DECISIONS[corrected-scheme-is-affordable-via-precomputed-stencil]` (graduated); `msc/research-structure-preserving/` stays live — its catalogue rows land here, its measured findings live in the eight structure-preservation DECISIONS entries, and its §4.8/§4.9 meat is owned by the two rows above.
