---
slug: detail-phenomena-systems-map
type: detail
...
# Phenomena systems map — inventory, build order, approach ledger

Source inventory for which Earth-lineage systems exist, on what timescales, how they couple multirate, and how each process wants to be modelled (approach codes). **Not claim law** for any single nomos; ordinum + nomotheke remain authority for charges/promises/status. Extracted from graduated DESIGN-SYSTEMS so the map is not dual-home prose.
...
3. **Build order (judgment, crude-rung fidelity per effort):** (1) erosion+hydrology done; (2) crude climate; (3) biomes; (4) weathering/pedogenesis; (5) vegetation; then feature systems; defer full GCM/ocean/speciation.
...
7. **Cordonnier/Braun prior art.** CG-terrain stream-power + implicit solve lineage (Galin review 2019; CBC\*16, CCB\*18) is the method family of the live erosion crude rung; multi-resolution space×time is the field's named open direction (seam architecture). Detail: `ref/research/` + graduated SYSTEMS anchor paragraph — not re-derived here.

8. **Out of bounds.** Quoting status columns as law; inventing phase content that contradicts `tabularium/terrestris.ordinum.udon`; promoting build-order as Joseph-ratified plan.

## Epistemic Status

**Max attainable: discussion-grade / empirical** for Earth timescales; **heuristic** for crude-payoff and build-order judgment. **Currently `discussion-grade`.** Stage `draft`. `detail` segment — inventory support for phase/nomos work.

## Discussion

Without a single map, agents re-invent system lists from memory and treat DESIGN-SYSTEMS status cells as claims. This segment keeps the inventory claim-adjacent while ordinum and core formulations own law.
