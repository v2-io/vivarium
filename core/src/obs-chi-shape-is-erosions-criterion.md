...
6. **But the offset does not track $(1+G)$, so the rate half stays a diagnostic.** The Davy–Lague reading — sediment re-laid must be re-cut, so the steepening should be $1+G$ — is refuted by the fourth column above, which declines monotonically from 1.00 to 0.75 instead of holding flat. The offset is monotone and tight in $G$ (per-tile spreads do not overlap between rows), so it is *calibratable*; it is not *derived*, and until it is, comparing the fitted slope to a reference is a measurement of the composition and not a gate on the landscape. Dividing instead by $(1+G/2)$ holds within 1.00–1.13 across the same rows — recorded as an **observed regularity with no derivation behind it**, whose candidate mechanism is that only part of the eroded volume is re-laid inside the network while the rest reaches an outlet and is lost to the sea. That mechanism is unverified and the regularity is not a law — and a derivation attempt has now made the fence load-bearing in both directions (`msc/research-lem-sota/chi-steepening-derivation-2026-07-28.md`): classical $1{+}G$ is *forced* by the continuous equations under the named deposit law and sediment conservation, outlet export included, and the measured sweep rejects it — so the offset is a **kernel-structure** effect (candidate culprits: sequential one-pass load, MFD-vs-D8 $A$ mismatch, hillslope terms), and no continuous algebra will close it. The gate path is instrument: a sediment-budget probe plus a hillslope-off control, re-measured on the post-fill-restore composition.
...
The third is how close the discrimination came to being misattributed, because the trail is the lesson. On the beacon patch the two forms were never asked to disagree, and the first premise statistic ($\sigma/\lvert\mu\rvert = 0.002$) blamed the driver for being too smooth — but that statistic was medianed over 377 no-test ocean tiles: *within fitted tiles* the driver varies by 24 % (FE(10)). What does not vary is $U$ *along a channel*, because no basin may exceed one tile; the whole-face control (FE(11)) removed that cap and the discrimination appeared on cue, 1.31–1.52× with the epoch-0 null passing. Uncorrected, the smooth-driver reading would have sent the fix to the uplift nomos. It belongs at the tile boundary — the same relocation, by the same instrument logic, as the lakes.
...
- **The fill repair moved the readings slightly and the criterion's power not at all.** With closed basins retained in the bed ( #obs-lakes-are-routed-over-not-carved-away ), the L13 beacon's stage-300 linearity residual $1-R^2(\sqrt{\ })$ reads 0.3077 against the immediately preceding cohort's 0.2882, and the zero-parameter ratio 0.9751 against 0.9193 — a landscape holding lakes is further from stream-power steady state than one whose basins were filled flat, so that is the sign to expect. Both sit far below the knickpoint amplitudes this segment convicts at, and the recorded per-epoch residual is unmoved (0.4109 against 0.4172 m/epoch). Quote the post-repair figures; the cohorts differ by whole-crate digest, so the attribution to retained basins is argued rather than isolated.
- **The varying-$U$ discrimination is measured and the blocker is named** (FE(10)–(11)). (The beacon-freeze mechanism cited around FE(4) is a *sink-policy, fine-grain* condition — at L9 the same policy grows the trunk 4.2× over 300 epochs, `#obs-lakes-are-routed-over-not-carved-away` FE(8) — so quote the freeze only with its grain.) What is *not* done: the whole-face experiment runs on an assembled network, so the honest next step is a builder that can carve a footprint whose basins are not capped — which is the seam work, not this segment's. Nothing here should be read as a case for assembling tiles at build time; the assembly is an instrument, and `#obs-tile-outlets-grade-away-the-basins` measures what it costs.
...
- **Not attempted: a declared threshold.** FE(12) and the Epistemic Status say why. The number to declare is roughly "twice the converged value for this composition", and nothing yet derives the converged value.
