...
10. **The uniform-$U$ premise holds under both drivers, and the second one holds it harder — which is a fact about the footprint, not about the driver.** The freeboard driver gives tile means 0.494–0.515 m/epoch at a within-tile $\sigma/\lvert\mu\rvert$ of 0.21; the column-derivative driver, which is signed and zero-mean globally ( #form-isostasy-column FE(7)), gives 0.666–0.687 at $\sigma/\lvert\mu\rvert$ of **0.002**, with no channel cell on non-positive uplift. The reason is that the new driver is coherent at continent wavelength while the beacon patch spans 313 km, so within one 78 km tile it is very nearly constant. So the shape residual is not an artifact of spatially varying $U$ under either driver, which is what lets FE(3)–(6) be read as being about the *composition*.

    **The consequence is that the discrimination this segment predicts has not yet been tested.** FE(2) exists because it survives a varying $U$ where the literature form does not, and neither cohort in the store varies $U$ enough for the two to part company. What would test it is a patch whose span is comparable to the driver's coherence length — a wider footprint, a coarser level, or a beacon placed across a craton margin — and none of those has been built.

11. **What this does not claim.** Not that a threshold has been declared — none has, and picking one is a nomos decision, not a probe's ( #form-time-indexed-stage-chains FE(4)). Not that the criterion is calibrated: FE(5) shows its converged value is set by $G$, and it would also move with $\kappa$, the talus angle, or the level. Not that the rate half is a gate — FE(6). Not a claim about any seed but this one, or any level but L13. Not that $\chi$-linearity holds for headwaters — it does not, and should not; the statistic improves monotonically with the channelization threshold (0.376 → 0.288 → 0.221 → 0.116 → 0.080 across 3, 10, 30, 100, 300 median cell areas), which is the stream-power law's own domain showing through.
...
**Currently `empirical`.** FE(3) is four cohorts under two drivers, which is replication across source trees and drivers but not across seeds, levels or footprints. FE(4)'s extension past the ladder is one tile per cohort. FE(5)–(6) are six tiles at one epoch count; the $(1+G/2)$ regularity is five points with no derivation and is labelled as such in the segment and in the probe's own output. Stage `draft`.

**Falsifiers.** A landscape run far past its response time whose shape residual does not fall refutes FE(3)–(4). A $G = 0$ configuration whose fitted slope does not recover $U/(k_{dt}A_0^{m})$ refutes FE(5) and would also convict the instrument. An inert tile reported as converged refutes FE(9). A `chi_profile` whose predicted profile is not a near-fixed-point of the incision solve refutes FE(2) — that one is convicted in the lib, not here.
...
- **The varying-$U$ discrimination is still untested, and needs a footprint rather than a driver** (FE(10)). A beacon spanning a craton margin, or the same measurement at L9 where a tile is 78 km × a much larger patch, would stress the premise. Until then FE(2)'s advantage over the literature form is argued from its algebra and shown only as a lower fit-noise floor, not demonstrated on a case where the literature form breaks.
