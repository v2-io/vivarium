...
6. **Null-space probe — instrument built, gate built, wiring open.** Periodic patch of a stencil; count zero eigenvalues of the discrete operator beyond rigid modes; extra zeros + eigenvectors name invisible modes (Cardiff). The can-fail gate exists at `examples/null_space_gate.rs` (reusing the full instrument's core by `#[path]`), with the two tolerance lessons baked into its source: f32 kernels need a $10^{-5}\lambda_{\max}$ cut, not $10^{-9}$, and cleanliness is judged by $\ge 4$ decades of daylight, never a fixed gap floor. Wired into `bin/check` (2026-07-29, runs in ~1 s release) — the guard is executed, not narration. Library-test lift = moving the ~200-line core to `src/probe/`, still open.
