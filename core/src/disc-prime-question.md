...
**Max attainable: robust-qualitative** as standing method for nomos work; the classical FD/linear modified-equation procedure is standard numerical analysis (not a project invention). Pedagogical framing "Prime Question" is project teaching language for that computation (`DECISIONS[structure-preserving-is-a-rediscovery-adopt-the-field]`, `:by claude`, `:status council-accepted` 2026-07-24 for LEXICON adoption — council-accepted sits strictly between proposed and decided and never upgrades `:by`; do not treat it as Joseph-settled law, but it is no longer a bare Claude proposal either). The requirement that the claim be *answerable before the scheme is sold as physics* rides with `DECISIONS[algorithms-are-disguised-physical-claims]` (`:by us`, decided) and with #norm-probes-before-claims (no behavior/physics claim without something that can fail).
...
- **LEXICON:** `modified-equation` / `backward-error-analysis` still unlanded; council-accepted (not Joseph-ratified) under the structure-preserving DECISION, not ratified here.
