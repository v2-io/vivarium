...
5. **First-light blocks on a cold heavyweight compute (never-block violation, 2026-07-24).** FE(4) says a view falls back to instant prior / coarser data and does **not** block. But the globe worker computes `sea_level::derived_sea_level_m` — an L8 pour of ~393k `lithosphere::freeboard_m` (isostasy) evaluations — **synchronously before the first frame**, and each per-request assemble also runs a full-face census pass. Since today's isostasy read made `freeboard_m` expensive, first light can hang for minutes at "pulling surface from store…" (only the sun HUD, which renders independently, shows) — worse under store/CPU contention with a running builder or probe. This is a cold *compute* on the critical path, not a store pull, so it is a real never-block breach. **View-side half landed (2026-07-24):** the globe now renders first light on a coarse view-side provisional sea (`provisional_sea_m`, an L4 pour over the pub `tectonic_surface_m`) and refines to the real L8 `derived_sea_level_m` on a side thread that self-pokes a rebuild — first light is instant (verified: 4.3 s whole-run wall vs the prior multi-minute hang), the HUD census labels `sea: PROVISIONAL → derived L8`, and the pour never sits on the frame path. **Store-side half still owed** (not done — borders the `sea_level`/builder path a live isostasy peer is reshaping, so `SRC_HASH` there is not churned mid-flight): memoize the derived sea into the store at `vivarium build` time (store-as-save) so no pour is needed per launch at all.
