...
5. **First-light cold-compute (never-block), closed both halves (2026-07-24).** FE(4) says a view falls back to instant prior / coarser data and does **not** block. The globe worker used to compute `sea_level::derived_sea_level_m` — an L8 pour of ~393k `lithosphere::freeboard_m` (isostasy) evaluations — **synchronously before the first frame**; since the isostasy read made `freeboard_m` expensive, first light could hang for minutes. **View-side half:** the globe renders first light on a coarse view-side provisional sea (`provisional_sea_m`, an L4 pour) and refines on a side thread — first light is instant, the pour never sits on the frame path. **Store-side half (landed 2026-07-24):** the per-epoch global scalars (derived sea + rock-mass-ledger integrals) are **store citizens** under complete keys (`query::World::epoch_reduction`, keyed through the `mantle-thermal` chain head; `#form-store-as-save` FE(6)). `vivarium build` materializes the whole epoch ladder; the globe warmer and the present-refine thread are store *readers* that prefer a Hit and prime the process's working-set staging caches from it, so a warmed process runs **no** pour at all (measured: the 7-epoch ladder warms in ~1.9 s cold vs ~0.85 ms once the store is built — `examples/epoch_store_timing.rs`). On a cold/absent store they compute off the frame path exactly as before, so the never-block posture is unchanged, only faster. The surface *field* is deliberately not stored — it regenerates O(1)/cell from the scalars at any resolution (thin-save, `#form-store-as-save` FE(7)).
