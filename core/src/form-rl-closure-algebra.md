...
5. **What this is not.** This segment does **not** claim that live kernels already implement honest $R \circ L = \mathrm{id}$, refluxing, or consumer-dependent $R$. It names the algebra the stack must speak. Method-zoo taxonomy (AMR, HMM, multirate, equation-free, …) is graduated teaching (`#form-rl-closure-algebra / #detail-seam-precedents`); primary-read seam precedents are held at #detail-seam-precedents (source graduated).

## Epistemic Status

**Max attainable: exact** for the operator vocabulary as shared mathematical structure (HMM $\mathcal{Q}\mathcal{R}=I$, equation-free $\mathcal{M}\mu=I$, Berger–Oliger injection — primaries read; see `#form-rl-closure-algebra / #detail-seam-precedents` (graduated teaching zoo), `#detail-seam-precedents / #form-seam-flux-exchange`, ARCHITECTURE §1).
...
1. **Mean-pin is not $R \circ L = \mathrm{id}$ on the mean** — and it **manufactures seam ridge and mass**. Measurement home: #obs-mean-pin-manufactures-seam (current-main, re-based land footprint: zero-physics ridge $\sim 2\times$; pin-on seam $12.81\to 3.84$ pin-off; mass $+0.217\%$ at $150$ fine epochs; residual median $0.489$ m). **Retirement executed** (`DECISIONS[mean-pin-retirement-executed-block-const-convicted-deletion-is-the-fix]`): the honest block-constant form pins the mean exactly (so law (1) is *satisfiable* by $h \leftarrow h + \Delta_{\mathrm{block}}$) but keeps the mass source and washboards the interior — convicted; deletion is the fix, with leaf-only + #form-face-flux-register the admissible replacement. Algebra law stands; implementation convicted.
...
Stage `draft`. Support: #detail-seam-precedents ; #post-represent-by-consequence . Do not treat uncorrected historical $R\circ L$ prose as this postulate.
...
- **Dual homes demoted (prior wave + this):** multiscale-methods **graduated** (`#form-rl-closure-algebra / #detail-seam-precedents`, 2026-07-24); ARCHITECTURE §1; multiscale-seams header + §1; DESIGN-REDUX §5 algebra join. Dynamic-exponent $z$ and reversion residue remain sketch/source until further segmented.
