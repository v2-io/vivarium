...
- **Sea is classified by elevation threshold, not connectivity — and Joseph's eye caught the consequence (2026-07-28, first depression-paint session).** Enclosed inland basins whose floors dip below the derived datum render as ocean and are excluded from depression capacity entirely, because `Fluvial::outlets` (and every reader downstream) marks any at-or-below-datum cell as sea-connected drain — landlocked or not. Real worlds keep endorheic seas below ocean level (the Caspian class); this world currently cannot express one. The repair direction is connectivity-aware sea classification (flood-fill from the true ocean rather than threshold), which touches the pour, outlets, and the paint classifications. Unmeasured: how many such basins exist and at what sizes — a census probe is the first convicting step. Sighting referent: Joseph's 2026-07-28 session, F0 L14 region window.
