...
4. **Freeboard may be negative.** Zero-mean isostatic stand-in (v0): long-wavelength fBm, can go negative. Full lithosphere thickness×density **column** isostasy is the target shape ( #form-isostasy-column ) — not required for first pour experiments (early-continents survey: magmatic/isostatic emergence precedes orogeny).
5. **Emerged land as flux.** The Abyssal gate is the flux quantity `emerged land`. The **isostasy** nomos **promises** it (keeper edge `:kept-by isostasy` in the ordinum, Claimed); erosion **consumes** it. Live: nomotheke closed; default builder runs erosion/water **without** `--allow-unmet`. Land fraction after pour is small (few-percent Abyssal band), not modern ~29%.
...
**Currently `robust-qualitative`:** pour, bathymetry split, freeboard keeper, and closed flux web are **live** under lib tests and smoke build without waiver. Residual: freeboard is not true column isostasy ( #form-isostasy-column ); pour area sampling residual; `SEA_LEVEL_M` still exists as a numeric residual for migrating call sites.
...
- **Full isostasy residual:** claim home #form-isostasy-column (shape accepted; column not built). Literature trail stays on that segment + early-continents survey.
