...
- **Owed decl upgrade (rock-mass ledger, 2026-07-24):** `LITHO_COLUMN`'s promise is `Conservation::NotTracked` ("becomes a real claim when transport/mass-return exists"). The mass-return now exists as a measured operator (`erosion_return.rs`, `#form-isostasy-column` FE(9)) with a closed-box conservation probe — so the honest edge becomes `Conserved`. Not flipped yet: the ledger is not the default surface (adoption step), and wiring it as its own nomos (consuming `EMERGED_LAND`, promising a rebounded/bounded-relief column with a `Conserved` stance) would otherwise be a dangling producer. Upgrade rides the adoption decision.
