...
3. **Per-epoch composition (order is load-bearing).** For each fluvial epoch on a face tile (`Fluvial::erode`):
   ...
   2. **Priority-Flood** depression fill with an $\varepsilon$-gradient (Barnes–Lehman–Mulla lineage; deterministic tie-breaks), producing a **routing surface**. Steps 3–6 are derived on it; the raise is **subtracted before step 7**, so talus, creep and the stored heights see the bed with its depressions intact ( #obs-lakes-are-routed-over-not-carved-away ). The fill also reports the true spill depth — the $\varepsilon$ excluded — which is what steps 5 and 6 consume to leave submerged cells un-incised and to settle arriving sediment into a lake bed rather than onto its surface. The three readers that call the same fill (`drainage_surface`, `chi_profile`, `response_census`) save and restore around it, as the loop now does.
   ...
   5. **Implicit stream-power incision** $E = K A^{m} S^{n}$ with **$n=1$** (Braun–Willett exact implicit solve; $m$ default $0.5$), **skipping submerged cells** — a river entering a lake sees the lake surface as base level, and the bed beneath it has no channel slope to act on.
   6. **Davy–Lague deposition** $D = G\cdot Q_s/A$ routed down the D8 tree; **in a submerged cell instead, full trapping into the bed** up to the remaining capacity, with the surplus spilling down the same tree ( #obs-lakes-are-routed-over-not-carved-away FE(4)).
...
6. **Measured / declared residuals (homes elsewhere; do not re-derive).**
   ...
   | Partial-tile **edge sinks** as base level | grades every tile to its own perimeter — tile-local basins capped at the tile (driver starved 3.85×), seam pits on assembly rising monotonically as the grain is made finer. **Grain-dependent, measured on both sides:** at the L9 planet build the two contracts agree within 10 % (1251 km tiles never reach their edges), while at the beacon footprint (78 km tiles) they end 280 m apart with 60.6 % of the tiled window in closed depressions — the 13 % figure was an edge-sink reader's understatement of the same quantity | #obs-tile-outlets-grade-away-the-basins FE(5) ; #obs-lakes-are-routed-over-not-carved-away FE(7) |
   | **Lake sediment trapping efficiency $=1$** | a submerged cell takes all arriving sediment up to its remaining capacity rather than a residence-time fraction, so basins silt as fast as supply allows and downstream reaches are starved by that much. Mass-conserving; the error is in the *rate* of infill | #obs-lakes-are-routed-over-not-carved-away FE(4) ; nomotheke unphysical term |
   | Priority-Flood $\varepsilon$-fill | **mass half retired** — the fill is a routing surface step (2) restores, so neither the spill fill nor the $\varepsilon$ reaches the bed. **Directional half stands**: the $\varepsilon$ orients flow across flats into long straight runs ( #obs-tile-outlets-grade-away-the-basins FE(8)) and incision is applied where flow goes | #obs-lakes-are-routed-over-not-carved-away FE(5) ; nomotheke unphysical term |
   ...
   | Uplift **rate** field is a carving driver, not a freeboard account | **signed and near-zero-mean** — the column derivative, with both rising and subsiding cells (`uplift::tests::rate_is_signed_and_near_zero_mean`), so this row no longer names a strictly-positive mass source. Freeboard remains isostasy's job | #form-isostasy-column ; live `uplift.rs` |
