...
**Currently `exact` as observation** of the live directional-fan defect (FE(1)). The **metric** fixes (true $A$, true neighbour lengths) are **live** in the fluvial kernel — no longer part of the open stack. The remaining **remedy stack** (edge-only FV routing; kill diagonals; moment conditions) is **proposed / unratified** — not project law. Cone-only “4× better router” headlines remain **gated** by a rotation test (Prescott) not yet claimed here.
