...
3. **Uniform cell area was a second cube-locked bias (partially retired).** Historical defect: drainage used `cell_area = cell_m²` — **one** area per level for the whole tile. Measured (pre-fix): correct at face centre; **$+41.2\%$** overstated $A$ at edge midpoints; **area-weighted mean $+17.810\%$** — **bit-identical at L5–L13** (no $N$). Stream power therefore saw a cube-locked fake erodibility. **Live (2026-07-23+):** drainage seed and deposit volume use true spherical area per cell (`measure::cell_area_m2`). **Still open:** MFD neighbour **lengths** remain uniform `cell_m` / diagonal $\sqrt{2}$ — geometry residual on the fan path. (`DECISIONS[drainage-area-uses-a-uniform-cell-area]`; PROBE 8 / measure tests.)
4. **Jacobian siblings.** The fan is the equiangular map’s **shear** showing through a flat-grid kernel; uniform area was the **determinant**. Closed-form area is live for $A$; router successor (edge fluxes; kill diagonals) remains open.
