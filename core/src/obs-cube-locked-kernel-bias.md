...
- **The accuracy fix has its own precision ceiling** (measured 2026-07-29, `examples/solid_angle_precision.rs`): `cell_area_m2` retired uniform `cell_m²` on accuracy grounds, and `cell_solid_angle`'s own *relative* precision degrades as $4^{\text{level}}$ because it cancels four O(1) arctangents — 1.6e-6 median at L19, 4.1e-4 at L23, 5.5e-3 median / 6.8e-2 max at L25. Live-kernel surface (per-cell runoff, deposition volumes). Drop-in fix identified (Van Oosterom–Strackee), **not landed** — it is a `src/` edit and re-keys every world; land it with the next deliberate kernel re-key (`DECISIONS[cell-solid-angle-loses-relative-precision-as-the-level-refines]`).
