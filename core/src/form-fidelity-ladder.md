...
9. **Corollary, weaker than FE(8) and still binding:** no derived physical quantity is computed on a mixed-tier surface, since such a figure measures the prior rather than the world. The tier census is not sufficient cover — a fraction in a text block does not scope a full-field number.
   ...
   **And fixing the reader alone is not sufficient — bed and field must come from the same rung.** Measured (`examples/lake_surface_probe` F, one L14 view over an L13 carve, 192² cells): a reader computing standing water *on the drawn surface* reports **4007 wet cells in 282 bodies**, against **806 in 26** when the same field is sampled from the carve — so the drawn surface invents 5× the wet area and 11× the bodies, and the 26 survivors are exactly the basins the kernel carved. But the sampled field is **not level over the drawn bed** (spread ~81–95 m against exactly 0.0 for the carve-consistent case), because carve-level water over a detail-augmented bed no longer conforms to the ground under it. The phantom does not go away when one consumer stops using it; it relocates to the disagreement between consumers. So the target is the *surface accessor*, not the reader.

   **A test can't catch a fabrication both sides share.** The drawn-surface reader passes the levelness test *perfectly* — bed and water came from the same phantom, and internal consistency with a fabrication is free. Levelness convicts the ε-augmented raise ( #obs-connectivity-fills-the-basins-the-threshold-drained FE(1)) and is silent on a phantom surface. Consistency checks bound arithmetic, not provenance; only the key can bound provenance.
...
- **FE(8) landed for the view paths (2026-07-30).** `erosion::surface_at_carved` feeds the mesh and the seam ghost; `erosion::prior_at_carve_level` is the change paint's baseline; the standing-water reader samples each region at its own level. Measured effect: the whole-globe depression paint went from 3761 wet cells / 1.258e6 km³ — on a view its own HUD called 100% uncarved prior — to **0**, and a lake over its own carved bed is level to the bit again where over a detailed bed it tilted ~90 m (changelog `2026-07-30-02`).
- **Instrument caption reconciled 2026-07-31 night.** Explore HUD `tier_line` (and pull/lens comments) still described the *retired* "prior detail re-added" mechanism after FE(8) landed — which re-taught the dishonesty the code had stopped doing. Present tense: coarser carve under a fine mesh = **stair-step at carve grain** (`surface_at_carved`); uncovered residual still falls to prior-at-view (unchosen policy). Convicting unit test: `fine_view_over_coarse_carve_depicts_carve_not_prior_detail`.
- **Still owed, and both are decisions rather than edits:** (i) uncovered cells still fall back to the prior at *their own* level, reintroducing bands no rung ran — a coarser band, or refusing to draw, is unchosen; (ii) `erosion::column_at` still builds columns from the detailed surface, which is this clause one layer down and in the object where every system meets.
