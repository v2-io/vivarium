...
- **Source peel:** `#detail-info-theoretic-discretisation / #detail-fvm-control-volume / #detail-structure-scheme-map` §3 present-best only; long correction blocks stay history in the graduated source.
...
- **Sweep breadcrumb (self-destructs).** `.super-archive/from-msc/redteam-discretisation/` — the discretisation red-team campaign. Likely primary absorbing segment: this one; the mapping is asserted from session memory, not verified — at the msc graduation sweep, *verify* the residue is fully absorbed here (or land what is missing, or re-point this note if the mapping is wrong), then graduate the residue to `.super-archive/` and **delete this note** (integration-is-replacement: the note's disappearance is the done-signal).
