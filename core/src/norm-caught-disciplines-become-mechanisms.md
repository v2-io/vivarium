...
- **Related specimen class, 2026-07-31 — inventable recovery prose.** Ambiguous `next:` / `next_action` strings (“run build”) were free-form and originless across status / HUD / capture. Same inventability shape as a digest, different content type: converted as **operator hints** with set/revoke edge log and `file:line` ( #ops-operator-hints ), not restated as “remember to suggest rebuild.”
