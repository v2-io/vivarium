...
The reason it is worth stating as law rather than fixing quietly in one function is that four separate places in the tree ask "is this cell ocean?" and each answers with its own threshold — routing, both paints, the terminal reader. Every one of them is now wrong in the same way the router was, and they cannot be repaired independently without four chances to disagree. The ocean set is a **world object** that the hydrosphere produces and other nomoi consume; it will be needed by more than hydrology (crustal columns distinguish ocean from continental basement, so plate work needs the same mask). FE(2) is written so that object can be lifted out of the router without restating the law.
