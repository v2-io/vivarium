...
- **Owed: promote the mask to a shared object.** `erosion::outlets` computes it privately per call. The consumers that still threshold independently: the explorer's surface and water paints, the terminal globe reader, and the water kernel's own initial fill. Until then, "ocean" means something slightly different in each, and only the router's meaning is right.
