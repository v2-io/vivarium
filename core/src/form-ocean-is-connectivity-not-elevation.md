...
5. **Role responsibilities (leakage → stricter split, not a softer claim).** When a world object **zaps under a pure view change** (zoom, patch size, camera) with no store write, that is **role leakage**: the view re-adjudicated something only the world may author. Standing split for **ocean**:
   - **World / nomos** — owns the ocean set as a **durable domain product** (face-or-coarser $\Omega$, complete key; recomputed when bed/sea change, not when the camera moves). Water-cycle and routing **consume** that product.
   - **Column** — **consumes** `is_ocean` (and freeboard); never invents ocean from height alone ( #form-column-control-volume ; `column_from_surface_at_sea_classified`). Ocean is not column-reduced.
   - **View** — **samples and depicts** the world's ocean bit (and may refuse honestly); must **not** re-run boundary-flood on the drawn postage stamp as if that were the planet's sea. Window-local `ocean_mask` on an assembled mesh tile is **paint convenience debt** until the face/store product lands — convicted live 2026-07-31 (close-in L14: blue interior appears/disappears with window size while HUD reports pure prior).
   - **Status / phase gate** — must not read as cycle-closed or morally green while ocean is view-conditional. Same discipline for air/weather later: domain transport and circulation are world articles; air columns hold vertical budgets and consume fluxes (Joseph 2026-07-31).

6. **What this does not settle.** Whether a landlocked basin actually *stands* full to its spill point is a water-balance question this formulation does not answer — see #obs-connectivity-fills-the-basins-the-threshold-drained FE(5) for the wet limit and what it assumes. This clause fixes only *what may hold water at all*.

7. **A residual seed defect, named and unrepaired.** When a domain contains no ocean under a no-flux wall, Priority-Flood still needs a seed, and the current rule makes the single **lowest** cell an outlet — which is always inside the deepest basin, so the deepest basin in a coastless walled window can never hold water. Views set a wall per drawn unit, so this reaches the paint on any drawn tile with no submerged cell touching its rim. Distinct from the classification error this segment repairs, and it is the next thing in this area to fix.

8. **Compute-bounded path (not a GCM).** Face-level (or coarser restriction) ocean mask memo under the bed/sea key — $O(\text{face cells})$ at the grain that adjudicates basins, **once per bed epoch**, not per frame. Close-in paint **samples** that field into the window ($O(n_x^2)$ lookup). Do not flood each explorer patch. Do not wait on full weather dynamics to fix FE(4) leakage.
...
**Currently `conditional`,** and the condition is FE(4): the identification of the computed mask with the planet's real ocean holds only where the domain contains the enclosing land ring. That is a genuine local assumption, not a hedge — at tile scale the mask is provably biased toward sea, and the bias direction is known. FE(5) role split and FE(8) compute path are **operational law for the repair**, not a claim that the face product is built. Stage `draft`.

The formulation is `formulation` rather than `derived` because a defensible alternative exists: a *designated* ocean region (a stored mask produced once at whole-planet scale and consumed by every reader) would remove FE(4)'s window dependence entirely. That is the better end state FE(5)/(8) name; shared pure-function mask is the half-step that landed 2026-07-31.
...
- **Shared object — landed 2026-07-31.** `sea_level::ocean_mask` / `is_submerged` are the pure functions; `Fluvial::outlets`, water initial fill + rim hold, terminal `globe`, explorer surface/water/provenance paints + mesh freeboard, and `column_at` / `column_from_surface_at_sea_classified` all consume them. Dual-arm unit test: `ocean_mask_is_connectivity_not_elevation`. **Still owed for full world-object status:** a store-memoized face/planet mask under a complete key (today the mask is recomputed from heights; explore still floods the **window** — FE(4)/(5) leakage). Legacy `column_from_surface_at_sea` still thresholds for pour-style tests — production paths use the classified form.
- **Joseph 2026-07-31:** amen that phase-3 `closed-water-cycle` must not read comfort-green while ocean zaps with zoom; role responsibilities on leakage (FE(5)); face-product path that does not burn the machine (FE(8)); status **honest red** with encouraging signs elsewhere.
- **Explore face-sample path — landed 2026-07-31:** `vivarium-explore` adjudicates ocean on whole-face heights at `min(view, 9)`, caches the mask, samples into close-in windows (unit tests: window flood invents ocean; face sample does not). **Not yet** a store citizen under a complete key — routing/water kernels still flood their own domains; paint is the first consumer of the face product.
...
- **Owed: FE(7)**, the coastless-wall seed. A candidate: give such a domain *no* outlet and let the fill raise everything to the rim, which is what a genuinely sealed basin does — but D8 needs a receiver, so this wants design rather than a one-line change.
