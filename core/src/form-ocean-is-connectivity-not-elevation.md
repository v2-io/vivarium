...
The reason it is worth stating as law rather than fixing quietly in one function is that **five** separate places in the tree ask "is this cell ocean?" and each answers with its own threshold — routing, both paints, the terminal reader, and the column constructor. Every one of them is now wrong in the same way the router was, and they cannot be repaired independently without five chances to disagree. The ocean set is a **world object** that the hydrosphere produces and other nomoi consume; it will be needed by more than hydrology (crustal columns distinguish ocean from continental basement, so plate work needs the same mask). FE(2) is written so that object can be lifted out of the router without restating the law.

## Working Notes

- **Owed: promote the mask to a shared object.** `erosion::outlets` computes it privately per call. The consumers that still threshold independently — **five**, not four as an earlier draft of this note said: the explorer's surface and water paints, the terminal globe reader, the water kernel's own initial fill, and **the column constructor itself** (`gen::column_from_surface_at_sea`, `let water = (sea_m - surface_m).max(0.0)`, which also sets pore `saturation` by the same test). The last is the one that matters most conceptually: the column is where every system meets, and it assigns its own standing water by the rule this segment retires. Until then, "ocean" means something slightly different in each, and only the router's meaning is right.
