...
**Current norm: sections partition by *subject*; epistemic weight is carried per-row (Type / status / stage), not by placement.** The reasoning: a sketch beside the law it extends is found by anyone reading that law, and its own label carries its weakness; filing by weight instead creates a dual taxonomy in which every promotion is a move and every subject needs two lookups — which is what produced both 2026-07-24 thrash specimens (phase physics fenced out of the phases; open research stowed in appendices as if non-primary, in a research program whose frontier *is* each subject's leading edge; Joseph corrected both). This is a norm with those specimens as its evidence, not a law beyond revisit: a future case where weight-placement genuinely serves findability should argue against the specimens, not against a slogan.

The one weight-defined section under the current norm is the **Appendix — retired paths**: no-gos, dead-end galleries, and seductive wrongs kept so nobody re-walks them. The cut it rests on: "retired" describes what the material *is about* (a path not to re-walk), where "unsettled" describes only how strongly we hold it — the row columns already say that.

Placement, in one pass: **reusable machinery for any multiscale world → §V toolkit** · **this world's systems, their laws, and what convicts their kernels → §VII Terrestris** (a conviction lives next to the kernel it convicts; phases say *what is charged when*, systems say *what the law is*, and a system that outlives its debut phase files by family, per §VII's header) · **store / pull / builder / engine without naming geology → §III machine** · **nomos grain, declaration, coupling → §II** · **telos, walls, norms, and purpose-adjacent bridges → §I** · **agents and inhabitation (the A axis, Level-C-gated) → §VIII** · **process instruments → §VI** · **retired → Appendix**. `depends:` stays the real prerequisite graph; keep onboarding un-inverted (don't seat §V-dependent material in §I), but outline order is taxonomy, not a proof sequence.
...
**Terrestris is the earthlike world engine.** Findings about its systems — water, erosion, geology, climate, materials as world substrate, phase build targets — live here, next to the phase ladder, **filed by system family** — nearly every system crosses phase boundaries, so family is the findable axis and phase is not (a genuinely phase-local system, if one appears, would file honestly under its phase head). Shared multiscale/grid *tools* stay in §V. Phases say *what is charged/promised when*; system segments say *what the law is* — including the system's own disposition changes across phases.
...
*Nearly every system crosses phase boundaries, so "cross-phase" discriminates nothing — systems file by **family**. A system's changing disposition across phases (dormant → crude → frozen → promoted) is a property of the system and lives on its segment; phase homes cite it via the ordinum join. The reasoning, which is also the revisit condition: restating a system per-phase shatters it into fragments a reader must reassemble — if a case appears where the citation direction costs more than the shatter, that is the argument to bring. Full inventory and build-order judgment: `#detail-phenomena-systems-map`.*
