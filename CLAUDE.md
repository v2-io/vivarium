...
See [`ORIENT.md`](ORIENT.md): `bin/try-me` issues the quiz as vanilla udon (`.orient/quiz.udon` + `|answer` stub) → fill answers → `bin/prove-me --ready …` → put the printed token in the commit message. Bare `prove-me` re-prints the quiz; `prove-me --help` never grades. (The gate is also a first real-world udon exercise.) Claim home: `#ops-orientation-gate` — after a pass, prefer segment/tool repairs and Working Notes backlog there when free-read or the quiz revealed friction.
...
| **Landing user-facing change** | [`changelog/`](changelog/) + `#ops-changelog-is-the-acceptance-check` **FE(7) checklist** — same cycle, do not soft-skip: pre-register → (if rebuild) commit world store pre/post (`~/.cache/vivarium/globe-world`) → capture → frozen entry with seed/`src=`/world-git revs/commands/`![[captures/…]]` embeds → commit entry+captures on source repo. Unwritable entry queues the work *or* the instrument |
