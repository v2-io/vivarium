...
pub fn moment_correct(g: &Mesh, i: usize, w: Vec<(usize, f64)>, psi: f64, e0: V3, e1: V3) -> Vec<(usize, f64)> {
    if w.len() < 2 {
        return w;
    }
    let c = g.centers[i];
...
pub fn lsq_grad(g: &Mesh, h: &[f64], i: usize) -> Option<(f64, f64, V3, V3)> {
    let c = g.centers[i];
...
pub fn weights(g: &Mesh, r: Router, h: &[f64], i: usize) -> Vec<(usize, f64)> {
    weights_with_psi(g, r, h, i, None, 1.1)
}

/// The local tangent frame at cell `i` — the SAME one `lsq_grad` and every router below use
/// (`e0` toward `adj[i][0]`, `e1 = c × e0`). Exposed so a probe can express an ORACLE ψ in the
/// frame the router will interpret it in. Deterministic: `adj[i][0]` is fixed at mesh build.
pub fn frame(g: &Mesh, i: usize) -> (V3, V3) {
    let c = g.centers[i];
    let e0 = tangent(c, g.centers[g.adj[i][0].j]);
    (e0, cross(c, e0))
}

/// `weights`, with two knobs a probe needs and the kernel must never have:
///
/// * `psi_override` — feed the router an EXACT flow azimuth instead of its own reconstructed
///   one. This is an **oracle**: it cheats by consulting the analytic answer, and exists only
///   to separate ψ-ESTIMATION error from WEIGHT-CONSTRUCTION error. Never a proposal.
/// * `p` — MFD's slope exponent, lifted out of the hardcode so it can be varied.
///
/// `psi_override` is interpreted in `frame(g, i)`. Routers with no ψ (`MooreMfd`, `EdgeMfd`)
/// ignore it, by construction — that is the point: they have no direction to correct.
pub fn weights_with_psi(
    g: &Mesh,
    r: Router,
    h: &[f64],
    i: usize,
    ...
    match r {
        ...
        Router::QuadMoment => {
            ...
            let psi = psi_override.unwrap_or_else(|| (-gy).atan2(-gx));
        ...
        Router::MomentMfd => {
            ...
            let psi = psi_override.unwrap_or_else(|| (-gy).atan2(-gx)); // azimuth of −∇h: where the water must go
        ...
        Router::GradEdgeFlux | Router::MomentEdge => {
            ...
            let mut gx = (syy * sxz - sxy * syz) / det;
            let mut gy = (sxx * syz - sxy * sxz) / det;
            // ⚠ ORACLE HOOK. The edge-flux weights are NORMALISED, so this router consumes the
            // gradient ONLY through its DIRECTION — its magnitude cancels exactly. Substituting
            // the exact downslope azimuth is therefore a complete removal of the ψ-estimation
            // error channel, changing nothing else about the scheme.
