...
> **Open decision blocking the world-level shape (Joseph's call — surface before building `regula.rs`).** The 2026-07-12 declarative-frontier spikes (`VIVARIA-DEFINITIONS.md` / `VIVARIA-DECLARATIVE-FRONTIER.md`) propose **collapsing the regula** into *order* (the descriptive `ordinum`) + *manifest* (per-world prescription: `:order`, `:target-phase`, permits, participation) — the conformance audit needs no `regula` artifact of its own. This is a PROPOSAL, not a decision (Joseph: the regula question is "small incidental"). Items 2/4/5 below assume regula-keep; if regula-collapse wins, permits move onto the manifest and `Slot` likely dissolves (a promise's fluxed-quantity + `:kept-by` already carries it). **Item 1 and the flux/requisite audit are common ground either way and are DONE (2026-07-12).**
