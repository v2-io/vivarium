...
# PROMOTED — use `#form-builder-admission` in core

**Promoted to** `core/src/form-builder-admission.md` (design-wave integration;
store sibling fixed to `#form-store-as-save`). This msc draft is no longer the
claim home.
...
1. **Two roles.** **Builder** advances a vivium by computing and putting memos under complete keys ( #form-complete-content-addressed-key ; sibling draft `#form-store-is-save` for save≡memo ontology). **Explorer / view** queries the store and never authors world-evolution parameters ( #form-core-view-wall ).
2. **Admission.** Before materializing a phase whose nomos has a requisite chain, the builder evaluates that chain against the nomotheke ( #form-flux-web ). If any quantity is **Unmet**, default admission **refuses** the phase — the world is mechanically unrunnable for that work, not "mostly fine."
...
4. **Observe-only pull.** View-facing surface queries prefer a store Hit of matured work (e.g. eroded tile) and otherwise fall back to instant prior / coarser lawful data — they do **not** cold-trigger long evolution computes that the builder has not done (`surface_prefer_eroded` and kin).
...
1. **Provisional artifacts look lawful.** Waiver is logged; store census / keys do not yet carry a provisional bit (de-novo audit residuals). Same incomplete as `#form-flux-web` (3).
...
- Promote only after orchestrator accepts slug name (alternative candidates: `form-builder-explorer-decoupling` — broader than built law).
- Ice dual-homes carefully: plan doc remains design for unbuilt daemon pieces; only the admission + observe-only pull claims move here.
