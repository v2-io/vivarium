...
| **a potential** — ⚠ **CORRECTED 2026-07-13, AND THE CORRECTION IS THE FINDING.** *"Gravity-driven flow is curl-free"* is **literally false**: real discharge is $q=-K\nabla\phi$ with varying conveyance, so $\nabla\times q = -\nabla K \times \nabla\phi \neq 0$. **Real water on a real hillslope HAS curl. A probe built on "curl-free" would have convicted a perfect router.** The **exact, pointwise, mesh-independent** statement is instead: **the flow direction is orthogonal to the contours of $\phi$** ⇒ $$\kappa(C)=\oint \hat d\cdot d\ell \Big/ \oint d\ell = \langle \sin\Delta\rangle = 0$$ **on every closed CONTOUR.** $\kappa$ is *the fraction of transport running **along** the contour instead of **down** the slope*. $\kappa \neq 0$ ⟺ **the flow spirals** ⟺ the routed field is not the gradient flow of the potential it claims to be. **An Escher staircase** | **mimetic / structure-preserving** discretisation (DEC, FEEC, compatible FD) — **plus, and this is the actual remedy, a FIRST-MOMENT CONDITION on the outflow weights: $\sum_k w_k \sin(\beta_k - \psi) = 0$** (*the tangential moment must vanish*) | **[M] ⛔ VIOLATED, AND MEASURED. $\kappa \approx 2.0\times10^{-2}$ — ~2% of the routed flux runs AROUND the hill rather than down it**, net of all cancellation. **Level-INDEPENDENT** (identical L9→L23, a 16 384× refinement in cell area) while the probe's own error falls five orders to machine zero. **The spurious vorticity is a smooth, coherent, SIGNED field** — CCW over one half of the face, CW over the other, strengthening outward. **It does not average out.** Refinement will never touch it | ⛔ `erosion` routing — **CONVICTED** |
