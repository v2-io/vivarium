...
Three laws keep the pair honest: (1) **$R \circ L = \text{id}$ on $U$** — reconstruct-then-summarize returns what the macro claimed (this *is* the fidelity invariant). ⚠ **CORRECTION (2026-07-13, measured): this law is stated as an ASPIRATION here, and the code does NOT satisfy it.** An earlier version of this line claimed *"mean-pinning a fine erosion tier to its parent is $R\circ L=\text{id}$ on the mean, live."* **That is FALSE.** `erosion.rs::pin_block_means` computes a block delta and then **bilinearly upsamples it**, which smooths across block boundaries and does **not** restore the block mean — the residual is exactly the $(1,6,1)/8$ stencil, i.e. $(\Delta_b^2/8)\nabla^2(\text{delta})$. **Measured on a real L19 eroded tile: mean residual 0.43 m, max 2.97 m at 30 epochs — growing to 1.93 m / 7.96 m at 150 epochs**, and **correlated with the surface curvature ($r=-0.42$)**, i.e. it tracks the very ridges and valleys erosion is producing. The in-tree guard `pin_preserves_parent_means` passes only because its tolerance (**2.0 m**) was sized to accommodate the defect, and its own comment admits the mechanism. *A block-constant delta injection would preserve the mean exactly and cost less.* See `DECISIONS[mean-pin-does-not-preserve-block-means]` and `doc/theory/discretisation-and-information.md`. **The same false claim appears in `doc/theory/multiscale-methods.md` §1 and `multiscale-seams.md` §2.1.** (2) **conservative flux exchange** — when a conserved quantity crosses a seam, exchange **fluxes, not states**, and they balance exactly; (3) **scale separation** — the fast process is *equilibrated* seen from above, the slow one *quasi-static* seen from below; **where separation fails, no multiscale method saves you** (the hydrology lesson: erosion is geological, water hydrological — never one timestep). Everything below is this algebra, made runnable.
