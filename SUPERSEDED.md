...
- **~~flat-root doc sprawl (15+ root markdown files; live design buried in ref/research/)~~ → the tiered tree** · 2026-07-11 (Joseph's reorg directive) · root = front doors + ethics/moratorium unit + ledgers; `doc/` = ARCHITECTURE/PHASES + design/ plan/ theory/; `ref/` = true reference; `.archive/` = accounted-for retirements (never referenced from the main tree). CLAUDE.md rewritten as tiered reading gates (Level A/B/C), retiring its stale "Stage (2026-06-29)" paragraph — which still called the archived Godot spike "the active first-person view." Norms instituted: `doc/PROCESS.udon` (with its feedback valve); toolchain decisions: `doc/toolchain.md` (uom = recorded skip). All moves `git mv`; cross-references updated in the same commit; link-check clean.
